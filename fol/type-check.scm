...
(define (check-expression-types expr env global-type defined-type-map #!optional proc)
  ...
  (define (check-lambda-types expr env)
    ...
    (let ((formal (cadr expr))
      ...
      (define declared-type (cadr type-decl))
      ...
      (if (not (eq? 'escaper (car canonical-type)))
          (error "Declaring LAMBDA to be other than an escaping function" declared-type))
      (define formal-types (escaping-function-arg-types canonical-type))
      (if (not (list? formal))
...
(define (equal-type? type1 type2 #!optional defined-type-map)
  ...
  (let loop ((type1 type1) (type2 type2))
    (cond ((and (function-type? type1) (function-type? type2))
          ...
          ((or (and (eq? 'escaping-function type1)
                    (escaping-function-type? type2))
               (and (escaping-function-type? type1)
                    (eq? 'escaping-function type2)))
           #t) ; TODO Flush this clause after all lambdas are required to be typed
          ((and (fol-var? type1) (or (eq? 'escaping-function type2)
                                     (not (fol-var? type2))))
           (if (not (equal? type1 (lookup type1)))
               (loop (lookup type1) type2)
               #f))
          ((and (or (eq? 'escaping-function type1)
                    (not (fol-var? type1))) (fol-var? type2))
