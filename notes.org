...
** TODO Write a test suite for my FOL implementation.
   Testing alpha-renamer can be fully automated (take a DVL program,
   compile it to raw FOL, feed to my optimizer, alpha-rename and check
   the result against Alexey's FOL using =alpha-rename?=).  What about
   inlining?  Is there a fully automatic way to test it?
